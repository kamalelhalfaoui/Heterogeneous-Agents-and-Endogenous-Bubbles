...
| Calibration | $\beta$ | $\varphi$ | $\alpha_{\text{opt}}$ | $\delta$ | $w$ | Noise | Purpose |
|---|---|---|---|---|---|---|---|
...
| Log returns $r_t$ | 55,000 periods | fat tails, near-zero return autocorrelation, positive and slowly-decaying $\lvert r\rvert$ autocorrelation (Cont, 2001) |
| Tail index | top 1–15% of $\lvert r_t\rvert$ | "cubic law", $\hat\zeta \approx 3$ |
...
$$
\max_{z}\ \ \mathbb{E}_{h,t}\!\left[W_{h,t+1}\right]
- \frac{a}{2}\,\mathbb{V}_{h,t}\!\left[W_{h,t+1}\right]
\tag{2}
$$
...
$$
R\,p_t = \sum_{h} n_{h,t}\,
\mathbb{E}_{h,t}\!\left[p_{t+1} + y_{t+1}\right]
\tag{4}
$$
...
$$
\textbf{Fundamentalists:}\qquad f_{h,t} = 0
\tag{8}
$$

$$
\textbf{Trend followers:}\qquad f_{h,t} = \alpha_h + \varphi_h\,x_{t-1}
\tag{9}
$$

$$
\textbf{Adapters:}\qquad f_{h,t} = \varphi_h\,x_{t-1} + \theta_h\,f_{h,t-1},
\qquad \varphi_h + \theta_h = 1
\tag{10}
$$

$$
\textbf{Learners:}\qquad f_{h,t} = \gamma_h\,(x_{t-1}-x_{t-2})
+ \xi_h\,\frac{\bar{x}_{t-1}+x_{t-1}}{2}
\tag{11}
$$

$$
\textbf{Optimists:}\qquad f_{h,t} = \alpha_h > 0
\tag{12}
$$

$$
\textbf{Pessimists:}\qquad f_{h,t} = \alpha_h < 0
\tag{13}
$$
...
$$
\hat{\zeta}^{-1} = \frac{1}{k}\sum_{i=1}^{k}
\ln \frac{\lvert r\rvert_{(i)}}{\lvert r\rvert_{(k)}}
\tag{22}
$$
...
$$
\hat\rho_k = \frac{\sum_{t=1}^{T-k}(z_t - \bar z)(z_{t+k} - \bar z)}
...
| Deterministic chaos | $\beta=10,\ \varphi=1.0$ | aperiodic | $\mathbf{+0.0864}$ | 2.227 | 6.454 |
...
| Mean $\lvert\hat\rho\rvert$ of $r_t$, lags 1–20 | 0.121 | **0.086** | $\approx 0$ |
| Mean $\hat\rho$ of $\lvert r_t\rvert$, lags 1–20 | $\mathbf{-0.018}$ | $\mathbf{+0.236}$ | positive, slow decay |
...
| **Adapters** | 0.059 | $\mathbf{-0.359}$ | 0.071 | $\mathbf{-0.362}$ |
...
| Configuration | $\sigma_x$ | Mean $\lvert x\rvert$ | Skewness | Excess kurtosis |
|---|---|---|---|---|
